...
with an FFI bridge to Dart.
  ...
  # Consume the iOS device / simulator slices of skylelib.xcframework instead of
  # compiling skylelib from source (same artifact the SwiftUI example links).
  # Resolution order (see darwin/skylelib_prebuilt.rb): SKYLELIB_DIST env → the
  # skylelib source tree's dist/ → download the GitHub release matching the
  # plugin version into <plugin>/.skylelib/ (pub git dependency path).
  # NOTE: resolve the Flutter plugin symlink to the real location.
  here     = File.realpath(__dir__)
  dist     = skylelib_resolve_dist(here)
  xcf      = File.join(dist, 'skylelib.xcframework')
  dev      = File.join(xcf, 'ios-arm64')
  sim      = File.join(xcf, 'ios-arm64_x86_64-simulator')
  ...
  s.pod_target_xcconfig = {
    ...
    'OTHER_LDFLAGS[sdk=iphoneos*]'        => "\"#{File.join(dev, 'libskylelib.a')}\"",
    'OTHER_LDFLAGS[sdk=iphonesimulator*]' => "\"#{File.join(sim, 'libskylelib.a')}\"",
