...
// Swift Package Manager manifest for the flutter_eap iOS plugin.
// Dual support: CocoaPods consumers keep using ../flutter_eap.podspec, which
...
//     values always equal the ones in ../../macos/flutter_eap/Package.swift.
...
let skylelibVersion = "1.5.0"
let skylelibChecksum = "06219544429376545d0e11bc4c92b3c1f679cc2e71e68eb9e55ca48b13118741"
...
let package = Package(
    ...
    products: [
        .library(name: "flutter-eap", targets: ["flutter_eap"])
    ],
    dependencies: [],
    targets: [
        // Swift plugin class (FlutterEapPlugin) plus the ExternalAccessory
        ...
        // (include/flutter_eap_ios.h) exposes only the functions
        // FlutterEapPlugin.swift needs. skylelib's namespaced headers
        // (<skylelib/...>) come from the binary target's Headers dir.
        .target(
            name: "flutter_eap_bridge",
            dependencies: [
                "skylelib"
            ],
            cSettings: [
                .define("EAP_PLATFORM_POSIX")
            ]
        ),
        skylelibTarget,
    ],
