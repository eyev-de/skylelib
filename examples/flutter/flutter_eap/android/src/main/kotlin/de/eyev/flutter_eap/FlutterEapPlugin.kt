...
class FlutterEapPlugin: FlutterPlugin, MethodCallHandler {
  ...
  companion object {
    ...
    //   <meta-data android:name="de.eyev.flutter_eap.AUTO_START_USB_HOST"
    //              android:value="false" />
    private const val META_AUTO_START_USB_HOST = "de.eyev.flutter_eap.AUTO_START_USB_HOST"

    // Skyle Link identity for SDK client apps (AUTO_START_USB_HOST=false):
    //   <meta-data android:name="de.eyev.flutter_eap.APP_ID"
    //              android:value="my-aac-app" />        (default: packageName)
    //   <meta-data android:name="de.eyev.flutter_eap.PRIORITY_TIER"
    //              android:value="1" />                 (default: 2; 0/skylex is reserved)
    private const val META_APP_ID = "de.eyev.flutter_eap.APP_ID"
    private const val META_PRIORITY_TIER = "de.eyev.flutter_eap.PRIORITY_TIER"
  ...
  private fun isAutoStartEnabled(context: Context): Boolean {
    return try {
      ...
      Log.w("FlutterEapPlugin", "Failed to read $META_AUTO_START_USB_HOST meta-data: $e")
      true
    }
  }
  ...
  private fun hasUsbPermission(context: Context): Boolean {
    return try {
      ...
      Log.w("FlutterEapPlugin", "hasUsbPermission check failed: $e")
      false
    }
  }
  ...
  override fun onAttachedToEngine(flutterPluginBinding: FlutterPlugin.FlutterPluginBinding) {
    ...
    // Skyle X path (EapUsbHost publishes the skylex identity itself) is
    // never double-driven.
    if (isAutoStartEnabled(flutterPluginBinding.applicationContext)) {
      ...
      Log.d("FlutterEapPlugin", "Plugin attached to engine - USB host auto-start disabled, enabling Skyle Link client supervisor")
  ...
  private fun enableClientSupervisor(context: Context) {
    try {
      ...
      EapClientJni.setIdentity(appId, tier, hasUsbPermission(context))
      EapClientJni.setSupervisorEnabled(true)
      Log.d("FlutterEapPlugin", "Skyle Link supervisor enabled (appId=$appId, tier=$tier)")
    } catch (e: Throwable) {
      // UnsatisfiedLinkError with an old libflutter_eap.so, or anything
      // unexpected: the app then simply sees no tracker.
      Log.e("FlutterEapPlugin", "Skyle Link supervisor setup failed: $e")
    }
  }
  ...
  override fun onMethodCall(call: MethodCall, result: Result) {
    when (call.method) {
      "configureTransport" -> {
        ...
        val ctx = context
        result.success(ctx != null && isAutoStartEnabled(ctx) && EapUsbHost.start(ctx))
      }
      ...
      "stopUsbHost" -> {
        ...
        // ownership. Best-effort and idempotent (see EapUsbHost.stop).
        EapUsbHost.stop()
        result.success(true)
      }
      "hasUsbPermission" -> {
