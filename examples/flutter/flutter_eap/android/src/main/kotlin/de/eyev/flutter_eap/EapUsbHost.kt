 ...
 * de.eyev.flutter_eap.AUTO_START_USB_HOST manifest meta-data (default true).
 *
 ...
 * bring-up (an old libflutter_eap.so degrades to plain USB).
 */
object EapUsbHost {
    ...
    // FlutterEapPlugin: APP_ID (default packageName) and PRIORITY_TIER
        ...
        synchronized(lock) {
            ...
            val manager = UsbEndpointManager(
                ...
                onUsbCapableChanged = { capable ->
                    ...
                    try {
                        EapClientJni.setIdentity(appId, tier, capable)
                    } catch (e: Throwable) {
            ...
            try {
                EapClientJni.setIdentity(appId, tier, manager.hasUsbPermission())
                EapClientJni.setUsbOwnershipListener(usbOwnershipListener)
                ...
                // UnsatisfiedLinkError with an old libflutter_eap.so, or
    ...
    fun stop() {
        synchronized(lock) {
            ...
            try {
                ...
                EapClientJni.setSupervisorEnabled(false)
            } catch (e: Throwable) {
            ...
            EapClientJni.setHostOwned(false)

            val clientPtr = EapClientJni.getInstance()
            if (clientPtr != 0L) {
                val result = EapClientJni.disconnect(clientPtr)
            ...
            try {
                EapClientJni.setUsbOwnershipListener(null)
            } catch (e: Throwable) {
