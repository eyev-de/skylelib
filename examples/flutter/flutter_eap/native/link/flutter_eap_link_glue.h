/**
 * @file flutter_eap_link_glue.h
 * @brief Shared Skyle Link glue for the flutter_eap platform shims.
 *
 * Compiled into every platform's shim library (Android: libflutter_eap.so via
 * android/CMakeLists.txt, iOS/macOS: the skylelib_unity.c unity include,
 * Windows: flutter_eap_plugin.dll via windows/CMakeLists.txt) so that Dart
 * resolves ALL Skyle Link symbols from the same library it already opens for
 * the flutter_eap_* API. This deliberately wraps the raw skyle_link_* /
 ...
 * them from the flutter_eap library on every platform.
 *
 ...
 * register (flutter_eap_fanout_dispatch_suspend_state on every fan-out
 * platform), delivering into each engine's subscriber slot. Dart seeds new
 * listeners from flutter_eap_get_suspension_state.
 */

#ifndef FLUTTER_EAP_LINK_GLUE_H
#define FLUTTER_EAP_LINK_GLUE_H
...
// Keep symbols reachable from Dart FFI (mirrors EAP_EXPORT in the Apple
...
#define EAP_LINK_EXPORT __attribute__((used)) __attribute__((visibility("default")))
#else
#define EAP_LINK_EXPORT
#endif
 ...
 */
typedef void (*flutter_eap_link_suspend_fanout_fn)(bool suspended, const char* holder);
 ...
 */
EAP_LINK_EXPORT void flutter_eap_link_glue_install(eap_client* client);
 ...
 */
EAP_LINK_EXPORT void flutter_eap_set_identity(const char* app_id, uint8_t tier, bool usb_capable);
 ...
 */
EAP_LINK_EXPORT void flutter_eap_set_supervisor_enabled(bool enabled);

/**
 * Current supervisor mode as int (skyle_link_supervisor_mode: 0 disabled,
 * 1 deciding, 2 owner, 3 client, 4 usb fallback).
 */
EAP_LINK_EXPORT int flutter_eap_get_supervisor_mode(void);
 ...
 * Returns eap_result as int.
 */
EAP_LINK_EXPORT int flutter_eap_link_set_suspended(eap_client* client, bool suspended);
 ...
 */
EAP_LINK_EXPORT void flutter_eap_get_suspension_state(bool* suspended, char* holder_buf, size_t buf_len);

#ifdef __cplusplus
}
#endif

#endif // FLUTTER_EAP_LINK_GLUE_H
