 ...
 * flutter_eap_fanout.h). Everything here is platform-neutral C17; the only
...
// stays "FlutterEapBridge": this IS the lifted bridge machinery, and the
...
static eap_subscriber g_subscribers[FLUTTER_EAP_MAX_SUBSCRIBERS];
...
// =============================================================================
//
// Installed on the core eap_client via install_core_adapters(); called by the
 ...
 * identical config).
 */
static int install_core_adapters(eap_client* client) {
...
int64_t flutter_eap_fanout_add(eap_client* client, const flutter_eap_callbacks* callbacks, int64_t engine_token) {
    ...
    if (engine_token != 0) {
        for (int i = 0; i < FLUTTER_EAP_MAX_SUBSCRIBERS; i++) {
            if (g_subscribers[i].handle != 0 && g_subscribers[i].engine_token == engine_token) {
                LOGD("flutter_eap_fanout_add: reaped stale subscriber handle=%lld (engine_token=%lld)",
    ...
    if (install_core_adapters(client) != 0) {
        flutter_eap_fanout_remove(handle);
        return -1;
    }
