/**
 * @file flutter_skyle_link_glue.c
 * @brief Shared Skyle Link glue implementation (all platforms).
 *
 * See flutter_skyle_link_glue.h for the architecture rationale. Everything in
...
#undef LOGD
#undef LOGE
#define LOG_TAG "FlutterSkyleLink"
...
// One-time adapter installation (flutter_skyle_link_glue_install).
...
// flutter_skyle_fanout_dispatch_suspend_state, delivering the change into each
...
static void link_glue_update_suspension(bool suspended, const char* holder) {
    ...
    flutter_skyle_link_suspend_fanout_fn fanout = g_suspend_fanout;
    if (fanout) {
        fanout(suspended, effective);
    }
}

void flutter_skyle_get_suspension_state(bool* suspended, char* holder_buf, size_t buf_len) {
    LINK_LOCK();
    if (suspended) {
...
// =============================================================================
// Client-mode adapter (installed on the client by flutter_skyle_link_glue_install)
// =============================================================================

static void link_glue_suspend_adapter(skyle_client* client, bool suspended, const char* holder_app_id, void* user_data) {
    (void)client;
    (void)user_data;
...
void flutter_skyle_link_glue_install(skyle_client* client) {
    if (!client) {
...
void flutter_skyle_set_supervisor_enabled(bool enabled) {
    skyle_link_set_supervisor_enabled(enabled);
    LOGD("set_supervisor_enabled: %d", enabled ? 1 : 0);
}

int flutter_skyle_get_supervisor_mode(void) {
...
int flutter_skyle_link_set_suspended(skyle_client* client, bool suspended) {
    if (!client) return -1;
