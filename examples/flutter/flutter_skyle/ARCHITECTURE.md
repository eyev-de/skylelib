...
```
Flutter/Dart
  SkyleClientFfi  (lib/src/ffi/skyle_client_ffi.dart)
...
C Library  (native/skylelib/)
    skyle_client  -- protocol parsing, state machine, heartbeat
...
2. **Message callbacks** -- parsed EAP messages, delivered to Dart through the
   ...
   `flutter_skyle_add_callbacks()` / `flutter_skyle_add_callbacks_engine()` and
   receives every callback independently (shared module:
   `native/fanout/flutter_skyle_fanout.c`). iOS stays single-slot
   (`flutter_skyle_set_callbacks()`, push mode, one engine).

## Shared Dart FFI Layer

All platforms share the same Dart code in `lib/src/ffi/skyle_client_ffi.dart`.

### Initialisation

1. Check `flutter_skyle_is_initialized()` to detect hot restart vs fresh start.
...
3. Allocate a `FlutterSkyleCallbacks` struct and fill it with the native function
   pointers from step 2. On the fan-out platforms register it as this engine's
   subscriber (`flutter_skyle_add_callbacks()` on Android, where Kotlin reaps a
   hot-restarted engine's stale handle, `flutter_skyle_add_callbacks_engine()` on
   desktop, where a re-add with the same engine token -
   `PlatformDispatcher.engineId`, which survives hot restart - reaps it
   natively); on iOS call `flutter_skyle_set_callbacks()`.
4. Call `_configureTransport()` via `MethodChannel('flutter_skyle/usb')` --
...
Dart: _bindings!.enableGaze(_clientPtr!, true)
  -> FFI call to C bridge: flutter_skyle_enable_gaze()
  -> C library: skyle_client_enable_gaze()
...
| Platform | Method |
|----------|--------|
| Android  | `DynamicLibrary.open('libflutter_skyle.so')` |
...
| File | Role |
|------|------|
| `ios/flutter_skyle/Sources/flutter_skyle/FlutterSkylePlugin.swift` | Plugin entry point, EAAccessory management, StreamDelegate |
| `ios/flutter_skyle/Sources/flutter_skyle/OutputStreamManager.swift` | Async output stream writer with backpressure |
| `ios/flutter_skyle/Sources/flutter_skyle/BoundedQueue.swift` | Thread-safe bounded FIFO queue |
| `darwin/Classes/flutter_skyle_bridge_apple.c` | C bridge (shared with macOS), push-mode functions |

### Connection Setup

1. `FlutterSkylePlugin.register()` registers for `EAAccessoryDidConnect` /
...
3. `configureTransport()` gets the singleton C client and calls
   `flutter_skyle_configure_push_transport()` with two Swift callbacks:
...
Skyle Device
  ...
  -> flutter_skyle_process_data(ptr, buffer, bytesRead)
  -> skyle_client_process_received_data() in C library
...
- A periodic `flutter_skyle_tick()` call (Swift timer, ~200ms) handles heartbeat
  and timeout detection.

### Session Lifecycle

- **Open:** `EASession(accessory:forProtocol:"de.eyev.eap")` -- opens streams,
  calls `flutter_skyle_connect()` which sets state to LINK_SYNCED.
- **Close:** `closeSession()` calls `flutter_skyle_disconnect()`, closes streams.
...
| File | Role |
|------|------|
| `android/.../FlutterSkylePlugin.kt` | Plugin entry, USB init, multi-engine support |
| `android/.../UsbEndpointManager.kt` | USB Host API: permissions, device open, bulk transfers |
| `android/.../SkyleClientJni.kt` | JNI function declarations |
| `android/src/main/cpp/flutter_skyle_bridge.c` | JNI transport callbacks + Dart adapters |
| `android/src/main/cpp/jni_bridge.c` | JNI entry points |

### Connection Setup

1. `FlutterSkylePlugin.onAttachedToEngine()` initialises USB once (first engine).
...
| File | Role |
|------|------|
| `macos/flutter_skyle/Sources/flutter_skyle/FlutterSkylePlugin.swift` | Plugin entry, triggers IOKit transport config |
| `darwin/Classes/flutter_skyle_bridge_apple.c` | C bridge (shared with iOS) |
| `native/skylelib/src/skyle_transport_iokit.c` | IOKit USB transport: device discovery, read, write |

### Connection Setup

1. Dart calls `configureTransport` via MethodChannel.
2. Swift calls `flutter_skyle_configure_iokit_transport(ptr, 0x3729, 0x7333)`.
3. C creates an `skyle_transport_iokit` struct, finds the USB device via IOKit
...
| File | Role |
|------|------|
| `windows/flutter_skyle_plugin.cpp` | Plugin entry, triggers WinUSB transport config |
| `windows/flutter_skyle_bridge_windows.c` | C bridge (Windows-specific) |
| `native/skylelib/src/rawbulk_winusb.c` | WinUSB USB transport: device discovery via SetupDi, read, write |
| `native/skylelib/src/skyle_transport_usb.c` | Transport wrapper around rawbulk (read/write/check callbacks) |

### Connection Setup

1. Dart calls `configureTransport` via MethodChannel.
2. C++ plugin calls `flutter_skyle_configure_usb_transport(client, 0x3729, 0x7333)`.
3. C creates an `skyle_transport_usb` struct which calls `rawbulk_open()`.
...
| **Bridge code** | darwin/flutter_skyle_bridge_apple.c | android/cpp/flutter_skyle_bridge.c | darwin/flutter_skyle_bridge_apple.c | windows/flutter_skyle_bridge_windows.c |
...
1. `SkyleClientFfi.create()` is called again after restart.
2. `flutter_skyle_is_initialized()` returns `true` (bridge context exists).
...
| File | Purpose |
|------|---------|
| `lib/src/ffi/skyle_client_ffi.dart` | FFI wrapper, NativeCallable listeners, StreamControllers |
| `lib/src/ffi/skyle_client_bindings.dart` | Generated FFI bindings to C functions |
| `lib/src/ffi/ffi_structs.dart` | Dart FFI struct definitions |
| `lib/src/skyle_client.dart` | High-level Dart API |

Riverpod providers live in the sibling `flutter_skyle_riverpod` package
(`lib/src/skyle_providers.dart` there).

### iOS

| File | Purpose |
|------|---------|
| `ios/flutter_skyle/Sources/flutter_skyle/FlutterSkylePlugin.swift` | EAAccessory management, StreamDelegate, push transport |
| `ios/flutter_skyle/Sources/flutter_skyle/OutputStreamManager.swift` | Backpressured async output stream writer |
...
| File | Purpose |
|------|---------|
| `darwin/Classes/flutter_skyle_bridge_apple.c` | C bridge: callback adapters, transport setup |
...
|------|---------|
| `windows/flutter_skyle_plugin.cpp` | Plugin entry, triggers WinUSB transport config |
| `windows/flutter_skyle_bridge_windows.c` | C bridge: callback adapters, transport setup |

### Native C Library

| File | Purpose |
|------|---------|
| `native/skylelib/src/skyle_client.c` | Core: state machine, background thread, message dispatch |
...
| `native/skylelib/src/skyle_transport_usb.c` | Transport wrapper around rawbulk (Windows) |
| `native/skylelib/src/skyle_transport_iokit.c` | IOKit USB transport (macOS) |
