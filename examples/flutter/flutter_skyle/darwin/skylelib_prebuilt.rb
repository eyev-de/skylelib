...
# skylelib.xcframework that the ios/ and macos/ flutter_skyle podspecs link.
#
...
def skylelib_pubspec_version(plugin_dir)
  ...
  raise 'flutter_skyle: could not parse version: from pubspec.yaml' unless v
  v
end
...
# all live outside it.
#
# The macOS podspec vendors into flutter_skyle/.skylelib (INSIDE the Swift
# package at macos/flutter_skyle) so the same copy doubles as the local
...
def skylelib_vendor_xcframework(podspec_dir, subdir = '.skylelib')
  ...
  raise "flutter_skyle: no skylelib.xcframework in #{dist}" unless File.directory?(src)
  require 'fileutils'
...
def skylelib_resolve_dist(podspec_dir)
  ...
  puts "flutter_skyle: downloading #{url}"
  system('curl', '-fsSL', '--retry', '3', '-o', zip, url) or
    raise "flutter_skyle: download failed: #{url}\n" \
          'Download/unzip the release yourself and set SKYLELIB_DIST to it instead.'
  system('unzip', '-q', '-o', zip, '-d', cache) or raise "flutter_skyle: could not unzip #{zip}"
  FileUtils.rm_f(zip)
  unless File.directory?(File.join(cache, 'skylelib.xcframework'))
    raise "flutter_skyle: #{asset} did not contain skylelib.xcframework"
  end
  cache
end
