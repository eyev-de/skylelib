# flutter_skyle — Public API Reference

**flutter_skyle** is the Flutter/Dart binding for **skylelib**, eyeV's
...
separate sibling package **flutter_skyle_riverpod**.

- **Package:** `flutter_skyle`
...
| Import | Exposes |
|--------|---------|
| `package:flutter_skyle/flutter_skyle.dart` | `SkyleClient` + all data models. The usual choice. |
| `package:flutter_skyle/flutter_skyle_models.dart` | Data models **only** — no client. Use in isolates / overlay windows to avoid accidental client init. |

Riverpod users: depend on the sibling package `flutter_skyle_riverpod` (same git
url/ref, path `examples/flutter/flutter_skyle_riverpod`) and import
`package:flutter_skyle_riverpod/flutter_skyle_riverpod.dart`, which re-exports the
full flutter_skyle API plus the providers — see
...
`SkyleClient` is the single high-level facade. It implements a set of segregated
interfaces so you (or Riverpod) can depend on just one capability:

`SkyleGaze`, `SkylePositioning`, `SkyleVersion`, `SkyleCalibration`, `SkyleVideo`,
`SkyleFileUpload`, `SkyleDeviceLogging`, `SkyleControl`.
...
Future<void> connect();     // start handshake + background I/O (throws SkyleException on failure)
...
| `deviceLogStream` | `Stream<SkyleLogMessage>` | Firmware log lines (only while device logging is on). |
| `logStream` | `Stream<SkyleLogMessage>` | Diagnostic logs from the Dart/FFI layer itself. |

### Feature commands

Each `enable*` is a no-op (logs a warning) unless `isReady`; the async
`Future`s complete once the request is queued and throw `SkyleException` on a
native error.
...
and **throws** `SkyleException` on failure/timeout (the timeout scales with file
...
Both deliver `SkyleLogMessage`.

---

## Riverpod providers

The providers live in the separate **flutter_skyle_riverpod** package, so apps
using other state managers can depend on flutter_skyle alone:
...
dependencies:
  flutter_skyle_riverpod:
    git:
      ...
      ref: vX.Y.Z   # same tag as flutter_skyle; it is resolved automatically
```

Import `package:flutter_skyle_riverpod/flutter_skyle_riverpod.dart` (re-exports
the full flutter_skyle API). All providers are `keepAlive` and share a single
`SkyleClient` that survives hot restart.

| Provider | Type | Description |
|----------|------|-------------|
| `skyleClientProvider` | `SkyleClient` | The singleton client (initialized for you). |
| `skyleClientInstanceProvider` | `SkyleClient` | Underlying notifier; `skyleClientProvider` proxies it. |
| `skyleControlProvider` | `SkyleControl` | Client as the control interface. |
| `skyleGazeProvider` | `SkyleGaze` | Client as the gaze interface. |
| `skylePositioningProvider` | `SkylePositioning` | Client as the positioning interface. |
| `skyleVersionProvider` | `SkyleVersion` | Client as the version interface. |
| `skyleVideoProvider` | `SkyleVideo` | Client as the video interface. |
| `skyleCalibrationProvider` | `SkyleCalibration` | Client as the calibration interface. |
| `skyleDeviceLoggingProvider` | `SkyleDeviceLogging` | Client as the device-logging interface. |
| `skyleGazeDataStreamProvider` | `Stream<GazesData>` | Gaze frames. |
| `skylePositioningDataStreamProvider` | `Stream<FaceData>` | Positioning frames. |
| `skyleVideoDataStreamProvider` | `Stream<VideoFrame>` | Video frames. |
| `skyleConnectionStateStreamProvider` | `Stream<ConnectionState>` | State changes. |
| `skyleConnectionStateProvider` | `ConnectionState` | Current state (defaults to `disconnected`). |
| `skyleControlDataStreamProvider` | `Stream<ControlData>` | Control updates. |
| `skyleCurrentControlDataProvider` | `ControlData` | Latest cached control state. |
| `skyleCalibrationStreamProvider` | `Stream<CalibrationMessage>` | Calibration events. |
| `skyleVersionDataProvider` | `Future<VersionData>` | Version info (waits for `linkSynced`). |
| `skyleErrorStreamProvider` | `Stream<String>` | Error messages. |
| `skyleLogStreamProvider` | `Stream<SkyleLogMessage>` | Dart-layer diagnostics. |
...
class SkyleLogMessage {
  ...
  final String source;            // e.g. "SkyleClient", or "device" for firmware lines
...
`synAckSent` (4), `connected` (5), `waitingStartAck` (6), `linkSynced` (7),
...
- Methods that hit the native layer throw **`SkyleException`** (with a `message`)
