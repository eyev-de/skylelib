...
# Run `pod lib lint flutter_skyle.podspec` to validate before publishing.
#
require File.join(File.realpath(__dir__), '..', 'darwin', 'skylelib_prebuilt.rb')

Pod::Spec.new do |s|
  s.name             = 'flutter_skyle'
...
with an FFI bridge to Dart.
  ...
  # package at flutter_skyle/Package.swift compiles the SAME files):
  # - flutter_skyle/Sources/flutter_skyle/FlutterSkylePlugin.swift
  #     (plugin registration + EASession push transport)
  # - flutter_skyle/Sources/flutter_skyle/OutputStreamManager.swift
  #   flutter_skyle/Sources/flutter_skyle/BoundedQueue.swift
  #     (backpressured async writer + its queue)
  # - flutter_skyle/Sources/flutter_skyle_bridge/include/flutter_skyle_ios.h
  #     (umbrella header for Swift-C interop)
  # - flutter_skyle/Sources/flutter_skyle_bridge/skylelib_unity.c
  #     (compiles only the shared Apple FFI bridge and the Skyle Link glue;
  #      skylelib itself is linked prebuilt, see below)
  s.source_files = 'flutter_skyle/Sources/**/*.{swift,h,c}'

  s.public_header_files = 'flutter_skyle/Sources/flutter_skyle_bridge/include/flutter_skyle_ios.h'

  s.dependency 'Flutter'
  # Floor of the prebuilt iOS slices (build_sdk.sh: IOS_DEPLOYMENT_TARGET=13.0),
  # matching flutter_skyle/Package.swift and Flutter's own iOS minimum.
  s.platform = :ios, '13.0'
  ...
  # copied into ios/flutter_skyle/.skylelib/ - INSIDE the Swift package, so the
  ...
  s.vendored_frameworks = skylelib_vendor_xcframework(here, File.join('flutter_skyle', '.skylelib'))
  dev = '${PODS_TARGET_SRCROOT}/flutter_skyle/.skylelib/skylelib.xcframework/ios-arm64'
  sim = '${PODS_TARGET_SRCROOT}/flutter_skyle/.skylelib/skylelib.xcframework/ios-arm64_x86_64-simulator'

  s.pod_target_xcconfig = {
    'DEFINES_MODULE' => 'YES',
    'EXCLUDED_ARCHS[sdk=iphonesimulator*]' => 'i386',
    # Bridge uses flat includes (<skyle_client.h>); shipped headers use namespaced
