...
/**
 * JNI bridge to the native flutter_skyle C library
 *
...
object SkyleClientJni {
    ...
    init {
        try {
            System.loadLibrary("flutter_skyle")
            Log.d("SkyleClientJni", "Loaded libflutter_skyle.so")
        } catch (e: UnsatisfiedLinkError) {
            Log.e("SkyleClientJni", "Failed to load libflutter_skyle.so", e)
            throw e
        }
    }
     ...
     *
     * @param clientPtr Native pointer to skyle_client
     ...
     *
     * @param clientPtr Native pointer to skyle_client
     * @return State value (see skyle_connection_state enum)
     */
     ...
     *
     * @param clientPtr Native pointer to skyle_client
     * @param handle Subscriber handle returned by flutter_skyle_add_callbacks
     */
     ...
     * suppresses disconnects. Used by SkyleUsbHost.stop().
     *
     * @param clientPtr Native pointer to skyle_client
     */
