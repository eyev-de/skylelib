...
(`SkyleClient`, streams, control settings, calibration, file upload and data
models). Riverpod providers live in the sibling
[flutter_skyle_riverpod](../flutter_skyle_riverpod) package.
...
- **State-manager agnostic**: Plain Dart streams; Riverpod providers available
  separately in [flutter_skyle_riverpod](../flutter_skyle_riverpod)
...
`flutter_skyle_add_callbacks_engine`, and a re-add with the same token replaces
the previous subscriber. The former desktop primary/passive engine guard
(`flutter_skyle_try_acquire_primary`) is gone - with per-engine subscribers
...
flow (one engine, `flutter_skyle_set_callbacks`).
...
[flutter_skyle_riverpod](../flutter_skyle_riverpod) package, so apps using other
state managers can depend on flutter_skyle alone. Depend on
`flutter_skyle_riverpod` and import
`package:flutter_skyle_riverpod/flutter_skyle_riverpod.dart` (it re-exports the
full flutter_skyle API); see its README for the provider list.
...
Moved to the sibling [flutter_skyle_riverpod](../flutter_skyle_riverpod) package;
...
enum ConnectionState {
  ...
  waitingStartAck,   // Waiting for the iAP2 session ACK
...
"local-link client" and uses the unchanged `SkyleClient` API. Protocol details:
...
SkyleClient.defaultIdentity = const SkyleLinkIdentity(appId: 'my-aac-app', priorityTier: SkyleLinkTier.partner);
```
...
<application>
    ...
    <meta-data
        android:name="de.eyev.flutter_skyle.AUTO_START_USB_HOST"
        android:value="false" />
    <!-- Skyle Link identity (defaults: packageName, tier 2): -->
    <meta-data
        android:name="de.eyev.flutter_skyle.APP_ID"
        android:value="my-aac-app" />
    <meta-data
        android:name="de.eyev.flutter_skyle.PRIORITY_TIER"
        android:value="1" />
</application>
```
...
The flutter_skyle plugin source code is licensed under the
