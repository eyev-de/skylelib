/**
 * @file flutter_skyle_bridge_windows.c
 * @brief FFI bridge implementation for Windows
 *
 * Architecture:
 *
 * Windows: USB transport via USB bulk endpoints
 *   - C++ plugin calls flutter_skyle_configure_usb_transport() for convenience
 ...
 *     fan-out (flutter_skyle_add_callbacks_engine, see
 ...
 *
 * Ported from flutter_skyle_bridge_apple.c with these changes:
 *   - skyle_transport_iokit -> skyle_transport_usb
...
#include <messages/skyle_message_types.h>
#include <skyle_transport_usb.h>
 ...
 * module (native/fanout/flutter_skyle_fanout.c) - this bridge keeps the WinUSB
 * transport/context specifics and delegates the callback machinery. */
typedef struct {
    skyle_client* client;
    skyle_transport_usb* usb_transport;
...
/* Guards the WinUSB transport lifecycle: flutter_skyle_configure_usb_transport
...
/* =========================================================================
 * Multi-engine subscriber fan-out (declared in flutter_skyle_fanout.h)
 * ========================================================================= */
 ...
 */
static int usb_register_transport_locked(bridge_context* ctx, skyle_client* client) {
    if (!ctx->usb_transport) {
        skyle_transport_usb_config config;
        memset(&config, 0, sizeof(config));
        config.vendor_id = ctx->usb_vendor_id;
        config.product_id = ctx->usb_product_id;
        config.timeout_ms = 1000;
        config.verbose = false;
        ctx->usb_transport = skyle_transport_usb_create(&config);
        if (!ctx->usb_transport) {
    ...
    transport_config.transport_write = skyle_transport_usb_write;
    transport_config.transport_read = skyle_transport_usb_read;
    transport_config.usb_device_check = skyle_transport_usb_get_check_callback();
    ...
    if (result != SKYLE_OK) {
        LOGE("usb_register_transport: skyle_client_set_transport failed (%d)", result);
        return (int)result;
    }
    return 0;
}
...
static void usb_ownership_callback(bool usb_wanted, void* user_data) {
    ...
    if (usb_wanted) {
        ...
        if (ctx->usb_transport) {
            skyle_transport_usb_destroy(ctx->usb_transport);
            ctx->usb_transport = NULL;
...
int flutter_skyle_configure_usb_transport(skyle_client* client, uint16_t vendor_id, uint16_t product_id) {
    if (!client) {
        LOGE("flutter_skyle_configure_usb_transport: NULL client");
        return -1;
    }
    ...
    if (result == 0 && !g_usb_ownership_registered) {
        ...
        LOGD("flutter_skyle_configure_usb_transport: USB ownership callback registered");
    }
    ...
    LOGD("flutter_skyle_configure_usb_transport: USB transport configured (VID=0x%04X, PID=0x%04X)", vendor_id, product_id);
    return 0;
}
...
int flutter_skyle_connect(skyle_client* client) {
    ...
    // supervisor-managed USB session); skyle_client_connect is a kick then.
